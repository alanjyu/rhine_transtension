/*
  Copyright (C) 2025 by Anne Glerum and Alan Yu.
  This file a plugin extension of ASPECT.
  ...
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
                            ...
                            std::vector<Vector<double> > &computed_quantities) const
      {
        ...
        // compressive stress
        // the fourth has a value of 1, 2 or 3, where 1 is normal faulting, 2 strike-slip and 3 thrust faulting
        // the fifth has a value of the Anglier stress ratio
        ...
        for (unsigned int q=0; q<n_quadrature_points; ++q)
          {
            ...
            std::tuple<Tensor<1,dim>, double, double> sigmaH_regime_ratio = compute_sigmaH_regime_ratio(compressive_stress, pressure, vertical_direction, orthogonal_directions);

            std::vector<double> sigma_H(dim,0.);

            for (unsigned int i=0; i<dim; ++i)
              {
                sigma_H[i] = (std::get<0>(sigmaH_regime_ratio))[i];
            ...
            computed_quantities[q](dim) = std::get<1>(sigmaH_regime_ratio);
            computed_quantities[q](dim+1) = std::get<2>(sigmaH_regime_ratio);
          }
      }

      
      template <int dim>
      std::tuple<Tensor<1,dim>, double, double>
      StressRegimeRatio<dim>::compute_sigmaH_regime_ratio(const SymmetricTensor<2,dim> compressive_stress,
                                                          const double pressure,
                                                          const Tensor<1,dim> vertical_direction,
                                                          const std::array<Tensor<1,dim>,dim-1 > orthogonal_directions) const
      {
        // initialize return values
        int stress_regime_zoback = -1;
        int stress_regime_anderson = -1;
        double differential_stress_ratio = std::numeric_limits<double>::quiet_NaN();
        double regime_stress_ratio = std::numeric_limits<double>::quiet_NaN();
        ...
        switch (dim)
          {
            ...
            case 2:
            {
              ...
              if (vertical_compressive_stress_magnitude > maximum_horizontal_compressive_stress_magnitude)
                stress_regime_zoback = 1;
              // thrust faulting
              else
                stress_regime_zoback = 3;
            ...
            case 3:
            {
              ...
              std::array<std::pair<double, Tensor< 1, dim, double> >,dim > eigen_vectors = eigenvectors(compressive_stress, SymmetricTensorEigenvectorMethod::hybrid);
                ...
                // Calculate differential stress ratio using the eigenvalues
                const double S1_val = eigen_vectors[0].first;
                const double S2_val = eigen_vectors[1].first;
                const double S3_val = eigen_vectors[dim-1].first;
              ...
              if (plunge_S1 >= 52.*degree_to_rad && plunge_S3 <= 35.*degree_to_rad)
                ...
                {
                  stress_regime_zoback = 5;
                  sigma_H_azimuth = azimuth_S1;
                }
              else if (plunge_S1 <= 35.*degree_to_rad &&
                       plunge_S3 >= 52.*degree_to_rad)
                {
                  stress_regime_zoback = 6;
                  sigma_H_azimuth = azimuth_S1;
                }
              ...
              if (std::abs(S1_val - S3_val) > 3.0 * std::numeric_limits<double>::epsilon())
                differential_stress_ratio = (S2_val - S3_val) / (S1_val - S3_val);
              else
                differential_stress_ratio = std::numeric_limits<double>::quiet_NaN();

              // normal faulting
              if (vertical_compressive_stress_magnitude > maximum_horizontal_compressive_stress_magnitude &&
                  maximum_horizontal_compressive_stress_magnitude > minimum_horizontal_compressive_stress_magnitude)
                stress_regime_anderson = 0.;
              ...
              // only compute the combined ratio if a valid regime was assigned (neq to -1)
              // and differential_stress_ratio is finite.
              if (stress_regime_anderson != -1 && std::isfinite(differential_stress_ratio))
                {
                  // stress_regime_anderson is an integer label; use parity to compute (-1)^n safely
                  const int regime_int = stress_regime_anderson;
                  const double sign = ((regime_int % 2) == 0) ? 1.0 : -1.0;
                  regime_stress_ratio = (static_cast<double>(stress_regime_anderson) + 0.5)
                                      + sign * (differential_stress_ratio - 0.5);
                }
              else
                {
                  // keep NaN to indicate undefined / unset
                  regime_stress_ratio = std::numeric_limits<double>::quiet_NaN();
                }
        ...
        return std::make_tuple(sigma_H, static_cast<double>(stress_regime_zoback), regime_stress_ratio);
      ...
      std::array<std::pair<double, Tensor<1, dim, double>>,dim>
        StressRegimeRatio<dim>::eigenvectors(const SymmetricTensor<2, dim, double> &T,
      ...
      StressRegimeRatio<dim>::get_names () const
      {
        std::vector<std::string> names(dim+2);
        names[0] = "sigma_H_x";
        names[dim-1] = "sigma_H_z";
        names[1] = "sigma_H_y";
        names[dim] = "stress_regime_zoback";
        names[dim+1] = "regime_stress_ratio";
        return names;
      }
      ...
      StressRegimeRatio<dim>::get_data_component_interpretation () const
      {
        std::vector<DataComponentInterpretation::DataComponentInterpretation> interpretation
        (dim+2,DataComponentInterpretation::component_is_part_of_vector);  // Need dim+2
        interpretation[dim] = DataComponentInterpretation::component_is_scalar;     // stress_regime_zoback
        interpretation[dim+1] = DataComponentInterpretation::component_is_scalar;   // regime_stress_ratio
      ...
      template <int dim>
      UpdateFlags
      StressRegimeRatio<dim>::get_needed_update_flags () const
      {
...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
      ASPECT_REGISTER_VISUALIZATION_POSTPROCESSOR(StressRegimeRatio,
                                                  ...
                                                  "field, along with stress regime classification and regime stress ratio. "
                                                  "A stress regime classification according to \\cite{Zoback92} is given as a number: "
                                                  "1 normal faulting, 2 normal to strike-slip faulting, "
                                                  "3 and 4 thrust faulting, 5 thrust to strike-slip and 6 thrust faulting. "
                                                  "A stress ratio of 1 indicates normal faulting, 2 strike-slip and 3 thrust faulting. "
                                                  "Recall that the "
